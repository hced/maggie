...
* **Fullscreen viewport at launch:** The viewport covers the **entire physical screen** from the very first presented buffer — including any shell bars (e.g., the Noctalia shell top bar) in the magnified capture — with **no compositor window rules** required. The layer surface explicitly requests the **full output logical size** (instead of relying on the compositor's initial size), **re-asserts it on every configure**, and **redraws immediately** whenever a configure changes the size.
